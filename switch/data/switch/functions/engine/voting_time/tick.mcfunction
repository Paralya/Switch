...
execute if entity @a[scores={switch.trigger.game_vote=..-1}] run scoreboard players remove #voting_timer switch.data 1
...
execute unless entity @a[scores={switch.trigger.game_vote=..-1}] run title @a actionbar [{"text":"Démarrage du compteur dès qu'un joueur vote.","color":"aqua"}]
execute if entity @a[scores={switch.trigger.game_vote=..-1}] if score #remaining switch.data matches 2.. run title @a actionbar [{"text":"Fin de la période de vote dans ","color":"aqua"},{"score":{"name":"#remaining","objective":"switch.data"},"color":"yellow"},{"text":" secondes."}]
execute if entity @a[scores={switch.trigger.game_vote=..-1}] if score #remaining switch.data matches 1 run title @a actionbar [{"text":"Fin de la période de vote dans ","color":"aqua"},{"score":{"name":"#remaining","objective":"switch.data"},"color":"yellow"},{"text":" seconde."}]

...
execute if score #game_state switch.data matches 1 run tellraw @a[scores={switch.lang=1},tag=!detached] ["\n",{"nbt": "Paralya", "storage": "switch:main", "interpret":true},{"text": "Game over, winners"},{"text": "red", "color": "red"},{"text":"! GG to "},{"selector":"@a[tag=!detached,gamemode=!spectator,team=switch.temp.red]"}]
execute if score #game_state switch.data matches 2 run tellraw @a[scores={switch.lang=1},tag=!detached] ["\n",{"nbt": "Paralya", "storage": "switch:main", "interpret":true},{"text": "Game over, GG win"},{"text": "blue", "color": "blue"},{"text":"! GG to "},{"selector":"@a[tag=!detached,gamemode=!spectator,team=switch.temp.blue]"}]

...
execute if score #sc_silencieux switch.data matches 1 run tellraw @a[scores={switch.lang=1},tag=!detached] [{"text":"➤ Silencer","color":"green","hoverEvent":{"action":"show_text","value":[{"text":"There's a 1/2 chance that a player's role won't be displayed when they die.\nThe scoreboard on the right is frozen.","color":"gray"}]}}]

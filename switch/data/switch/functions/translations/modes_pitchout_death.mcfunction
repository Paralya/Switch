...
# English
execute if entity @s[scores={switch.alive=1}] run tellraw @a[scores={switch.lang=1},tag=!detached] [{"selector":"@s","color":"dark_red"},{"text":" died permanently, he survived"},{"score":{"name":"#pitchout_seconds","objective":"switch.data"}},{"text":" seconds!"}]
...
execute if entity @s[scores={switch.alive=3}] run tellraw @a[scores={switch.lang=1},tag=!detached] [{"selector":"@s","color":"red"},{"text":"fell, only 2 lives left!"}]

...
execute if score #infected_seconds switch.data matches 10 run tellraw @a[tag=!detached] [{"selector":"@p[scores={switch.lang=0,switch.temp.infection=1}]","color":"green"},{"text":" a été choisi comme premier infecté !","color":"dark_purple"},{"text":"\n"},{"text":"Humains, les renforts arrivent dans moins de 10 minutes !","color":"light_purple"}]

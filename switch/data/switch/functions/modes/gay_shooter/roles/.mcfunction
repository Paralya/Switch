...
execute if score #next_role switch.data matches 1 run tag @s add switch.gay_shooter.spectre
execute if score #next_role switch.data matches 2.. run team join switch.temp.visible @s
execute if score #next_role switch.data matches 2.. run tag @s add switch.gay_shooter.visible
...
execute as @e[tag=switch.new] run function switch:modes/gay_shooter/roles/marker

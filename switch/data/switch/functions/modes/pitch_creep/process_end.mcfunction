...
execute if score #process_end switch.data matches 1 run advancement grant @a[tag=!detached,scores={switch.alive=1..,switch.temp.shot=0}] only switch:visible/59
execute if score #process_end switch.data matches 1 if entity @a[scores={switch.alive=1..}] run tellraw @a[tag=!detached] ["",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Fin du mini-jeu ! Les joueurs suivants ont remporté la partie : "},{"selector":"@a[scores={switch.alive=1..}]"}]

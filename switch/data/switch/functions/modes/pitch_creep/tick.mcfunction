...
execute as @a[tag=!detached,x=0,y=69,z=0,distance=..10] run function switch:modes/pitch_creep/death
execute at @e[type=marker,tag=switch.selected_map,limit=1] run kill @a[tag=!detached,distance=50..,scores={switch.alive=1..}]

execute unless entity @a[tag=!detached,scores={switch.alive=1..}] run function switch:modes/pitch_creep/process_end

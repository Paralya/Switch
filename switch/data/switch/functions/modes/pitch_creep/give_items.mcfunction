...
execute as @e[type=item,tag=!switch.done,distance=..1,nbt={Item:{id:"minecraft:diamond_sword"}}] run data modify entity @s Item.tag.Enchantments append value {id:"minecraft:knockback",lvl:5}
execute as @e[type=item,tag=!switch.done,distance=..1,nbt={Item:{id:"minecraft:bow"}}] run data modify entity @s Item.tag.Enchantments append value {id:"minecraft:punch",lvl:5}

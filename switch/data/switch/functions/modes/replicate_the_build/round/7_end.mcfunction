...
execute as @a[tag=!detached,scores={switch.alive=1..}] if score @s switch.rtb.points = #min_score switch.data run tag @s add switch.looser_score
...
execute as @a[tag=!detached,scores={switch.alive=1..}] if score @s switch.rtb.time = #max_time switch.data run tag @s add switch.looser_time

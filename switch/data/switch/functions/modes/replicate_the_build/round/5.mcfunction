...
scoreboard players add @a[tag=!switch.detached,scores={switch.alive=1..},gamemode=survival] switch.rtb.time 1

execute if score #rtb_build_time switch.data matches ..0 run function switch:modes/replicate_the_build/round/5_end
execute if score #rtb_round_state switch.data matches 5 unless entity @a[tag=!switch.detached,gamemode=!spectator,scores={switch.alive=1..}] run function switch:modes/replicate_the_build/round/5_end

...
execute as @a[tag=!switch.detached,x=0,y=69,z=0,distance=..10] run function switch:modes/replicate_the_build/death


# Fin de la partie si il n'y a plus de joueur en vie, ou que le temps est écoulé
execute unless entity @a[tag=!switch.detached,scores={switch.alive=1..}] unless score #rtb_round_state switch.data matches 9 run function switch:modes/replicate_the_build/process_end

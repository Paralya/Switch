...
execute as @p[gamemode=!spectator,scores={switch.temp.role=1..2},distance=..1] run tellraw @a {"text":"L'arc du détective a été ramassé !","color":"green"}
execute as @p[gamemode=!spectator,scores={switch.temp.role=1..2},distance=..1] as @a at @s run playsound entity.experience_orb.pickup ambient @s

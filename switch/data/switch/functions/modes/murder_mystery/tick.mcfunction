...
execute if score #murderer_reload switch.data matches 188 as @a[tag=!detached,scores={switch.temp.role=3}] at @s run function switch:modes/murder_mystery/throw/throw
execute if score #murderer_reload switch.data matches 1..170 as @a[tag=!detached,scores={switch.temp.role=3,switch.right_click=1..}] at @s run function switch:modes/murder_mystery/throw/error
execute if score #murderer_reload switch.data matches ..0 as @a[tag=!detached,scores={switch.temp.role=3,switch.right_click=1..},nbt={SelectedItem:{id:"minecraft:golden_sword"}}] run function switch:modes/murder_mystery/throw/init

...
execute as @e[tag=iris.ray] run function switch:modes/murder_mystery/throw/as_iris_ray
...
execute as @e[type=item_display,tag=switch.thrown,sort=nearest,limit=1] run data modify entity @s item.tag.destination set from storage switch:temp

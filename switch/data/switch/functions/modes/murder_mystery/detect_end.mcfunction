...
execute if entity @a[gamemode=!spectator,scores={switch.temp.role=3}] unless entity @a[gamemode=!spectator,scores={switch.temp.role=1..2}] run scoreboard players set #game_state switch.data 1
execute if entity @a[gamemode=!spectator,scores={switch.temp.role=1..2}] unless entity @a[gamemode=!spectator,scores={switch.temp.role=3}] run scoreboard players set #game_state switch.data 2
...
execute if score #remaining_time switch.data matches ..1 if score #game_state switch.data matches 1 run tellraw @a ["\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Fin de partie, victoire de "},{"selector":"@a[gamemode=!spectator,scores={switch.temp.role=3}]","color":"red"}]
execute if score #remaining_time switch.data matches ..1 if score #game_state switch.data matches 2 run tellraw @a ["\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Fin de partie, victoire des "},{"text":"innocents","color":"green"},{"text":" ! GG à "},{"selector":"@a[gamemode=!spectator,scores={switch.temp.role=1..2}]"}]
execute if score #remaining_time switch.data matches ..1 if score #game_state switch.data matches 3 run tellraw @a ["\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Fin de partie, aucun des camps n'est sorti vainqueur car il n'y a plus aucun joueur en vie !"}]

execute if score #remaining_time switch.data matches ..1 if score #game_state switch.data matches 0 as @a[gamemode=!spectator,scores={switch.temp.role=1..2}] at @s run function switch:engine/add_money
...
execute if score #remaining_time switch.data matches ..1 as @a[scores={switch.temp.role=3}] on attacker if entity @s[scores={switch.temp.role=1}] run tellraw @a ["",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Hero : "},{"selector":"@s","color":"blue"}]

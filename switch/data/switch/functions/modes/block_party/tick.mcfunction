...
## Death system
execute as @a[tag=!switch.detached,gamemode=!spectator,sort=random] at @s if block ~ ~ ~ moving_piston run function switch:modes/block_party/death
...
execute if score #block_party_ticks switch.data matches 1000.. run stopsound @a[tag=!switch.detached] record
...
execute store result score #remaining_players switch.data if entity @a[tag=!switch.detached,gamemode=!spectator]
execute if score #remaining_players switch.data matches ..1 run function switch:modes/block_party/process_end
execute if score #block_party_seconds switch.data matches 1000.. run function switch:modes/block_party/process_end

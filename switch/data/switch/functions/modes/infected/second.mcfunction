...
execute if score #infected_seconds switch.data matches ..-2 run scoreboard players operation #temp switch.data = #infected_seconds switch.data
execute if score #infected_seconds switch.data matches ..-2 run scoreboard players operation #temp switch.data *= #-1 switch.data
execute if score #infected_seconds switch.data matches ..-2 run title @a[tag=!detached] actionbar [{"text":"Début de la partie dans ","color":"dark_purple"},{"score":{"name":"#temp","objective":"switch.data"},"color":"light_purple"},{"text":" secondes","color":"dark_purple"}]
...
execute if score #infected_seconds switch.data matches 25.. as @p[tag=switch.to_infect] run tellraw @a[tag=!detached] [{"selector":"@p[tag=switch.to_infect]","color":"dark_purple"},{"text":" a été choisi comme nouvel infecté car il n'y a plus de zombies !"}]

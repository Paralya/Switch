...
execute if score #process_end switch.data matches 1 if score #game_state switch.data matches 0 as @a[team=switch.temp.human] at @s run function switch:engine/add_money
execute if score #process_end switch.data matches 1 if score #game_state switch.data matches 1 as @a[team=switch.temp.zombie,scores={switch.temp.original_zombie=1}] at @s run function switch:engine/add_money

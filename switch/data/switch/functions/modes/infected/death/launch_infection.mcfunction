...
execute if entity @p[tag=switch.temp] unless score @s switch.temp.infection matches 2.. run tellraw @a ["",{"selector":"@s"},{"text":" vient de se faire infecter par ","color":"green"},{"selector":"@p[tag=switch.temp]"},{"text":" !","color":"green"}]

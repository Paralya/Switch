...
execute store result score #votes switch.data if entity @a[tag=!detached,scores={switch.temp.change_map=1}]
...
tellraw @a[tag=!detached] [{"selector":"@s","color":"green"},{"text":" a voté pour changer de map ["},{"score":{"name":"#votes","objective":"switch.data"},"color":"aqua"},{"text":"/"},{"score":{"name":"#votes_max","objective":"switch.data"},"color":"aqua"},{"text":"]"}]

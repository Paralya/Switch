...
execute if score #remaining_time switch.data matches 61.. if entity @a[tag=!switch.detached,scores={switch.alive=1}] run tellraw @a[tag=!switch.detached] ["",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Vous n'avez plus que 60 secondes pour terminer la course !\n"}]

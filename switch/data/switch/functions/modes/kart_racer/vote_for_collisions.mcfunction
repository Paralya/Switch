...
execute store result score #votes switch.data if entity @a[tag=!switch.detached,scores={switch.temp.vote_collisions=1}]
...
tellraw @a[tag=!switch.detached] [{"selector":"@s","color":"red"},{"text":" a voté pour activer les collisions joueurs ["},{"score":{"name":"#votes","objective":"switch.data"},"color":"aqua"},{"text":"/"},{"score":{"name":"#votes_max","objective":"switch.data"},"color":"aqua"},{"text":"]"}]

execute if score #votes switch.data >= #votes_max switch.data run scoreboard players set @a[tag=!switch.detached] switch.temp.vote_collisions 1
execute if score #votes switch.data >= #votes_max switch.data run tellraw @a[tag=!switch.detached] ["\n",{"nbt":"ParalyaWarning","storage":"switch:main","interpret":true},{"text":" Collisions entre joueurs activées !\n"}]

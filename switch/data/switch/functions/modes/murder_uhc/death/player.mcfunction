...
tag @s[tag=!switch.murder_uhc.second_life] remove switch.alive
...
tellraw @s[tag=switch.murder_uhc.second_life] ["\n",{"nbt":"ParalyaWarning","storage":"switch:main","interpret":true},{"text":" Vous est mort mais votre capacité de "},{"text":"ninja","color":"green"},{"text":" vous permet de revenir dans la partie !"}]
attribute @s[tag=switch.murder_uhc.second_life] generic.max_health base set 10.0
attribute @s[tag=switch.murder_uhc.second_life] generic.movement_speed modifier add 1-1-1-1-1 "Ninja" 0.15 multiply_base
tag @s[tag=switch.murder_uhc.second_life] add switch.ninja_death
tag @s[tag=switch.murder_uhc.second_life] remove switch.murder_uhc.second_life

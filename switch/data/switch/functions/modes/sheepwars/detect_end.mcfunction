...
execute if score #game_state switch.data matches 1 as @a[tag=!detached,gamemode=!spectator,team=switch.temp.red] unless score @s sheepwars.launched_count matches 1.. run advancement grant @s only switch:visible/11
execute if score #game_state switch.data matches 2 as @a[tag=!detached,gamemode=!spectator,team=switch.temp.blue] unless score @s sheepwars.launched_count matches 1.. run advancement grant @s only switch:visible/11

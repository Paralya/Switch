...
effect give @a[tag=!switch.detached] hunger 5 255 true
...
effect give @a[tag=!switch.detached] darkness 5 255 true
effect give @a[tag=!switch.detached] jump_boost 5 250 true
effect give @a[tag=!switch.detached] slowness 5 255 true
...
execute if score #APOCALYPSE_GAME switch.data matches 1 run tellraw @a[tag=!switch.detached] ["\n",{"nbt":"ParalyaWarning","storage":"switch:main","interpret":true},{"text":" PARTIE APOCALYPTIQUE (1 chance sur 20) !\n"}]
execute if score #APOCALYPSE_GAME switch.data matches 1 as @a[tag=!switch.detached] at @s run playsound entity.wither.death ambient @s
...
execute as @a[tag=!switch.detached,sort=random] at @s run function switch:modes/sheepwars/team_and_give

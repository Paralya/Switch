...
# Détection des morts
execute as @a[tag=!switch.detached,x=0,y=69,z=0,distance=..10,sort=random] run function switch:modes/sheepwars/death
effect give @a[tag=!switch.detached,predicate=switch:in_water,nbt=!{active_effects:[{id:"minecraft:wither"}]}] wither 2 2 true

# Tick du sheepwars
function sheepwars:tick
kill @e[type=arrow,nbt={inGround:1b}]
kill @e[type=item,nbt=!{Item:{tag:{}}}]
execute as @a[tag=!switch.detached,nbt=!{foodLevel:20}] run effect give @s saturation 1 0 true

# Force offhand item
execute as @a[tag=!switch.detached,nbt=!{Inventory:[{Slot:-106b,id:"minecraft:warped_fungus_on_a_stick"}]}] run item replace entity @s[nbt={Inventory:[{Slot:-106b}]}] hotbar.8 from entity @s weapon.offhand
clear @a[tag=!switch.detached,nbt=!{Inventory:[{Slot:-106b,id:"minecraft:warped_fungus_on_a_stick"}]}] warped_fungus_on_a_stick
item replace entity @a[tag=!switch.detached,nbt=!{Inventory:[{Slot:-106b,id:"minecraft:warped_fungus_on_a_stick"}]}] weapon.offhand with warped_fungus_on_a_stick{CustomModelData:2010003,Unbreakable:1b}
...
execute if score #sheepwars_night switch.data matches 1 run tellraw @a[tag=!switch.detached] [{"text":"[SheepWars] ","color":"yellow"},{"text":"Une nuit intergalactique commence !","color":"blue"}]
...
execute if score #sheepwars_night switch.data matches 1 as @a[tag=!switch.detached] at @s run playsound entity.wither.spawn ambient @s

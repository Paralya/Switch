...
execute if entity @a[scores={switch.alive=1..},team=switch.temp.visible] unless entity @a[scores={switch.alive=1..},team=switch.temp.spectre] run scoreboard players add #game_state switch.data 1
execute if entity @a[scores={switch.alive=1..},team=switch.temp.spectre] unless entity @a[scores={switch.alive=1..},team=switch.temp.visible] run scoreboard players add #game_state switch.data 2
...
execute if score #game_state switch.data matches 2 run tellraw @a ["\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Fin de partie, victoire des "},{"text":"spectres","color":"yellow"},{"text":" ! GG à "},{"selector":"@a[scores={switch.alive=1..},team=switch.temp.spectre]"}]
...
execute if score #game_state switch.data matches 1 as @a[scores={switch.alive=1..},team=switch.temp.visible] at @s run function switch:engine/add_money
execute if score #game_state switch.data matches 2 as @a[scores={switch.alive=1..},team=switch.temp.spectre] at @s run function switch:engine/add_money

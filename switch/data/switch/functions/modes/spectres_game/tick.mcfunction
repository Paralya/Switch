...
execute as @a[tag=!switch.detached,tag=switch.to_tp] run function switch:modes/spectres_game/teleport_to_death
kill @e[type=item,nbt={Age:200s}]

execute as @a[tag=!switch.detached,x=0,y=69,z=0,distance=..10] run function switch:modes/spectres_game/death/player
...
execute at @a[tag=!switch.detached,team=switch.temp.spectre,gamemode=!spectator,predicate=!switch:is_sneaking,predicate=!switch:in_air] run particle dolphin ~ ~ ~ 0.2 0 0.2 0 2 normal

# On enlève les particules d'absorption
execute as @a[tag=!switch.detached,nbt={active_effects:[{id:"minecraft:absorption",show_particles:1b}]}] run function switch:modes/spectres_game/absorption

# Particules sur tous les spectres, pour les spectres
execute at @a[tag=!switch.detached,gamemode=!spectator,team=switch.temp.spectre] run particle dust 1 1 0.5 1 ~ ~2.1 ~ 0.2 0 0.2 0 1 force @a[tag=!switch.detached,team=switch.temp.spectre]

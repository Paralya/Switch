...
execute if score #random switch.data matches 11 if entity @s[team=switch.temp.visible] run tellraw @a[tag=!detached] ["",{"text":"Tasks Game","bold":true,"color":"green"},{"text":" \u2022 ","bold":true,"color":"gray"},{"text":" Un visible aléatoire est devenu très puissant temporairement !","color":"light_blue"},{"text":"\n "}]

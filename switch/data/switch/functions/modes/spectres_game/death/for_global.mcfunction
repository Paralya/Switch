...
execute if score #success switch.data matches 1 if entity @s[scores={switch.alive=3}] run tellraw @a[tag=!detached] ["",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Le joueur "},{"nbt":"data.Name","entity":"@s"},{"text":" est mort, il était un "},{"text":"Spectror","color":"red"},{"text":" !"}]

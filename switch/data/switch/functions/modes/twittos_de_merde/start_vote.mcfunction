...
execute if score #phase_de_vote switch.data matches 0 run tellraw @a[tag=!detached] {"text":"Levez les mains ! Les tweets arrivent !\n","color":"red"}
execute if score #phase_de_vote switch.data matches 0 run scoreboard players set #timer_phase_de_vote switch.data 10
execute if score #phase_de_vote switch.data matches 0 run scoreboard players set @a[tag=!detached,scores={switch.alive=1..}] switch.temp.passage_vote 1
execute if score #phase_de_vote switch.data matches 0 run scoreboard players set #twittos_battle_state switch.data 1
execute if score #phase_de_vote switch.data matches 0 run execute as @a[tag=!detached] at @s run playsound entity.player.levelup ambient @s
...
execute if score #timer_phase_de_vote switch.data matches 10.. if entity @a[tag=!detached,scores={switch.temp.passage_vote=0..1}] run function switch:modes/twittos_de_merde/calculation_vote
execute if score #timer_phase_de_vote switch.data matches 10.. if entity @a[tag=!detached,scores={switch.temp.passage_vote=0..1}] run function switch:modes/twittos_de_merde/tweet_affichage
execute unless entity @a[tag=!detached,scores={switch.temp.passage_vote=0..1}] run function switch:modes/twittos_de_merde/get_rank

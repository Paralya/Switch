...
execute unless entity @a[tag=!switch.detached,scores={switch.temp.passage_vote=0..1}] run function switch:modes/twittos_de_merde/theme_aleatoire
...
#scoreboard players operation #max switch.data > @a[tag=!switch.detached] switch.temp.rank
#execute as @a[tag=!switch.detached] if score @s switch.temp.rank = #max switch.data run tag @s add switch.looser

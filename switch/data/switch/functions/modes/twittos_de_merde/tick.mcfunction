...
execute unless entity @a[tag=!detached,scores={switch.alive=1..}] run function switch:modes/twittos_de_merde/process_end
execute if score #remaining_time switch.data matches 0 run function switch:modes/twittos_de_merde/process_end
execute if entity @a[tag=!detached,scores={switch.temp.passage_vote=0..1}] run scoreboard players enable @a[tag=!detached] switch.temp.rating_vote

...
execute at @a[tag=!detached,scores={switch.alive=3}] run particle dust 0 .75 0 1 ~ ~2.4 ~ .1 .1 .1 0 1
...
scoreboard players remove @a[tag=!detached,scores={switch.temp.cooldown=1..}] switch.temp.cooldown 1
item replace entity @a[tag=!detached,scores={switch.temp.cooldown=0}] armor.chest with air

...
execute if score #pitchout_seconds switch.data matches 100 as @a[tag=!detached,scores={switch.alive=1..3}] unless score @s switch.temp.damages matches 1.. run advancement grant @s only switch:visible/13

...
execute if score #sc_silencieux switch.data matches 1 run tellraw @a[tag=!detached] [{"text":"➤ Silencieux","color":"green","hoverEvent":{"action":"show_text","value":[{"text":"Il y a 2/3 chance que le rôle d'un joueur ne soit pas affiché lors de sa mort.\nN'affecte pas les messages de l'inspecteur.\nLe scoreboard à droite est gelé.","color":"gray"}]}}]

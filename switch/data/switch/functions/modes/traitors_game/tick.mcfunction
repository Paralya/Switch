...
execute as @a[tag=!switch.detached,tag=switch.to_tp] run function switch:modes/traitors_game/teleport_to_death
execute as @a[tag=!switch.detached,tag=switch.ninja_death] run function switch:modes/traitors_game/death/to_tp

# Glow particles pour le détective, particules rouges pour traitres
execute at @a[tag=!switch.detached,gamemode=!spectator,scores={switch.temp.role=1}] run particle glow ~ ~ ~ 0.2 0 0.2 0 5
execute at @a[tag=!switch.detached,gamemode=!spectator,scores={switch.temp.role=5..6}] run particle dust 1 0 0 1 ~ ~2.1 ~ 0.2 0 0.2 0 1 force @a[tag=!switch.detached,scores={switch.temp.role=5..6}]

execute as @a[tag=!switch.detached,x=0,y=69,z=0,distance=..10] run function switch:modes/traitors_game/death/player

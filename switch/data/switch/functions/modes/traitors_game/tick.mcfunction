...
execute at @a[gamemode=!spectator,scores={switch.temp.role=1}] run particle glow ~ ~ ~ 0.2 0 0.2 0 5
execute at @a[gamemode=!spectator,scores={switch.temp.role=5..6}] run particle dust 1 0 0 1 ~ ~2.1 ~ 0.2 0 0.2 0 1 force @a[scores={switch.temp.role=5..6}]

execute as @a[x=0,y=69,z=0,distance=..10] run function switch:modes/traitors_game/death/player

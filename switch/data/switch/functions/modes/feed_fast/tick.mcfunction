...
# Pour chaque joueur,
execute as @a[gamemode=!spectator] run function switch:modes/feed_fast/tick_player

# Fin de la partie si il n'y a plus de joueur en vie, ou que le temps est écoulé
execute if score #remaining_time switch.data matches 1.. unless entity @a[gamemode=!spectator] run scoreboard players set #remaining_time switch.data 0

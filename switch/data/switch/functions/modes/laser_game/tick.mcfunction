...
scoreboard players add @a[scores={switch.temp.dead_cooldown=..-1}] switch.temp.dead_cooldown 1
execute as @a[scores={switch.right_click=1..},sort=random] at @s run function switch:modes/laser_game/right_click
tag @a[tag=global.ignore,scores={switch.temp.dead_cooldown=0}] remove global.ignore

...
execute as @a[gamemode=adventure,sort=random] at @s if entity @s[y=0,dy=121] run function switch:modes/tnt_run/death

# Summon a marker if players are on the ground
execute if score #tnt_run_seconds switch.data matches 0.. as @a[gamemode=adventure] at @s run function switch:modes/tnt_run/is_on_ground

...
execute if entity @s[tag=switch.first_life,tag=!switch.second_life] run tellraw @a [{"selector":"@s","color":"red"},{"text":" est mort définitivement, il a survécu "},{"score":{"name":"#spleef_seconds","objective":"switch.data"}},{"text":" secondes !"}]
...
execute if entity @s[tag=switch.first_life] if data storage switch:main {map:"spleef_1"} run function switch:modes/spleef/map_1/teleport_players

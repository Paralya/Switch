...
execute as @a[tag=!switch.detached,gamemode=!spectator,gamemode=!creative] at @s if block ~ ~-1 ~ barrier run kill @s
execute as @a[tag=!switch.detached,x=0,y=69,z=0,distance=..10] run function switch:modes/creeper_apocalypse/death
...
title @a[tag=!switch.detached] actionbar [{"text":"Creepers : ","color":"dark_green"},{"score":{"name":"#creeper_count","objective":"switch.data"},"color":"green"},{"text":", Temps restant : "},{"score":{"name":"#remaining_time","objective":"switch.data"},"color":"green"},{"text":"s","color":"dark_green"}]

# Fin de la partie si il n'y a plus de joueur en vie, ou que le temps est écoulé
execute unless entity @a[tag=!switch.detached,scores={switch.alive=1..}] run function switch:modes/creeper_apocalypse/process_end

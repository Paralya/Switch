...
execute if score #process_end switch.data matches 1 run scoreboard players set #add_override switch.money 50
execute if score #process_end switch.data matches 1 as @a[tag=!detached] at @s run function switch:engine/add_money

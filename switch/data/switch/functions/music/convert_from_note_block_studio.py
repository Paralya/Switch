...
execute if score @s switch.music.progress matches 1.. run tellraw @s [{"text":"}
...
""")
	...
	lines = []
	for i, objective in enumerate(objectives):
		lines.append(f"""execute if score @s switch.music.current matches {i+100} run tellraw @s [{{"text":"➤ {objective[0].replace("_"," ").title()}","color":"#FFC0CB","clickEvent":{{"action":"run_command","value":"/trigger switch.trigger.music set {i+100}"}}}}]""")
		lines.append(f"""execute unless score @s switch.music.current matches {i+100} run tellraw @s [{{"text":"➤ {objective[0].replace("_"," ").title()}","color":"light_purple","clickEvent":{{"action":"run_command","value":"/trigger switch.trigger.music set {i+100}"}}}}]""")
	browser_file.write("\n".join(lines) + "\n\n")

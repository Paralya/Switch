...
data modify storage switch:temp input set value {tick:0,name:"stay_inside_me"}
...
execute if score #temp switch.data >= #stay_inside_me switch.music.progress run scoreboard players set @s switch.music.current -1
execute if score #temp switch.data >= #stay_inside_me switch.music.progress run scoreboard players set @s switch.music.progress 0

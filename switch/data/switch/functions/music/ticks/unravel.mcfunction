...
data modify storage switch:temp input set value {tick:0,name:"unravel"}
...
execute if score #temp switch.data >= #unravel switch.music.progress run scoreboard players set @s switch.music.current -1
execute if score #temp switch.data >= #unravel switch.music.progress run scoreboard players set @s switch.music.progress 0

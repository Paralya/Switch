...
def generate_clone_survival_folder(name: str, start_pos: tuple, end_pos: tuple, paste_start_height: int, kart_racer: list = [], override_tp_coords: tuple = None):
	...
	for k in splitted_coordinates:
		dx = (k[2] - k[0]) // 2
		dz = (k[3] - k[1]) // 2
		marker_file.write(f"execute if score #rg_{name}_mod switch.data matches {i} at @s in switch:game run particle cloud {k[0] + dx} ~{dy + 0.5} {k[1] + dz} {dx} 0 {dz // 2} 0 {particle_count} force\n")
		marker_file.write(f"execute if score #rg_{name}_mod switch.data matches {i} at @s run clone from minecraft:overworld {k[0]} ~ {k[1]} {k[2]} ~ {k[3]} to switch:game {k[0]} ~{dy} {k[1]} replace force\n")
		i += 1
...
generate_clone_survival_folder("sky_tower", (80000, -64, 80000), (80142, 0, 80142), 150, override_tp_coords = (80071, 173, 80071))
...
generate_clone_survival_folder("even_old_japan", (162000, 0, 162000), (162102, 52, 162102), 100, override_tp_coords = (162038, 109, 162043))
generate_clone_survival_folder("fireblast_1", (163000, 0, 163000), (163075, 50, 163075), 100, override_tp_coords = (163037, 127, 163037))
generate_clone_survival_folder("fireblast_2", (164000, 0, 164000), (164075, 50, 164075), 100, override_tp_coords = (164037, 127, 164037))
generate_clone_survival_folder("fireblast_3", (165000, 0, 165000), (165075, 50, 165075), 100, override_tp_coords = (165037, 127, 165037))
...
""")
	for name in survival_maps:
		f.write(f"data modify storage switch:maps to_scan.{name} set value 1b\n")
...
## This file is generated by the 'generate_survival_folder.py' script
## Do not edit this file but the script instead
execute if data storage switch:maps to_scan{{{survival_maps[-1]}:true}} run schedule function switch:maps/loop_scan_doors_of_every_maps 1t
execute if data storage switch:maps to_scan{{{survival_maps[0]}:true}} run function switch:maps/survival/{survival_maps[0]}/scan_doors
""")
	for i in range(1, len(survival_maps)):
		previous_map = survival_maps[i - 1]
		current_map = survival_maps[i]
		f.write(f"execute unless data storage switch:maps to_scan.{previous_map} if data storage switch:maps to_scan{{{current_map}:true}} run function switch:maps/survival/{current_map}/scan_doors\n")


## Generate a file that launch the regeneration on every map progressively
with open("regenerate_every_maps.mcfunction", "w", encoding = "utf-8") as f:
	f.write("""
## This file is generated by the 'generate_survival_folder.py' script
## Do not edit this file but the script instead
...
""")
	...
	f.write("schedule function switch:maps/loop_regenerate_every_maps 1t\n")
with open("loop_regenerate_every_maps.mcfunction", "w", encoding = "utf-8") as f:
	f.write(f"""
## This file is generated by the 'generate_survival_folder.py' script
## Do not edit this file but the script instead
execute if data storage switch:maps to_regenerate{{{survival_maps[-1]}:true}} run schedule function switch:maps/loop_regenerate_every_maps 1t
execute if data storage switch:maps to_regenerate{{{survival_maps[0]}:true}} run function switch:maps/survival/{survival_maps[0]}/regenerate
""")
	for i in range(1, len(survival_maps)):
		previous_map = survival_maps[i - 1]
		current_map = survival_maps[i]
		f.write(f"execute unless data storage switch:maps to_regenerate.{previous_map} if data storage switch:maps to_regenerate{{{current_map}:true}} run function switch:maps/survival/{current_map}/regenerate\n")

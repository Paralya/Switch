...
generate_clone_survival_folder("de_a_coudre_1", (28968, 95, 28971), (29051, 192, 29031), 95, override_tp_coords = (29000, 101, 29000))
generate_clone_survival_folder("mlg_a_coudre_1", (30968, 95, 30955), (31056, 220, 31049), 95, override_tp_coords = (31000, 101, 31000))
...
""")
	for i in range(1, len(survival_maps)):
		...
		f.write(f"execute unless data storage switch:maps to_scan.{previous_map} if data storage switch:maps to_scan{{{current_map}:1b}} run function switch:maps/survival/{current_map}/scan_doors\n")
...
""")
	for i in range(1, len(survival_maps)):
		previous_map = survival_maps[i - 1]
		current_map = survival_maps[i]
		f.write(f"execute unless data storage switch:maps to_regenerate.{previous_map} if data storage switch:maps to_regenerate{{{current_map}:1b}} run function switch:maps/survival/{current_map}/regenerate\n")

...
# Fireball
execute as @a[tag=!detached,gamemode=adventure,scores={switch.right_click=1..},nbt={SelectedItem:{components:{"minecraft:custom_data":{}}}}] if score @s switch.temp.cooldown matches 1.. at @s run function switch:modes/fireblast/fireball/no_cooldown
execute as @a[tag=!detached,gamemode=adventure,scores={switch.right_click=1..},nbt={SelectedItem:{components:{"minecraft:custom_data":{}}}}] unless score @s switch.temp.cooldown matches 1.. at @s run function switch:modes/fireblast/fireball/right_click

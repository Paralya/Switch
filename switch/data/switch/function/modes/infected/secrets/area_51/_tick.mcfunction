...
execute if score #infected_secret switch.data matches 0..2 unless entity @e[type=item,tag=switch.infected.bottle] run summon item 2006 151 2018 {Tags:["switch.infected.bottle","switch.infected.item"],Item:{id:"minecraft:glass_bottle",Count:1b,components:{"minecraft:item_name":'{"text":"Test tube","italic":false,"color":"light_purple"}',"minecraft:lore":['{"text":"Infected","italic":true,"color":"dark_green"}']}}}

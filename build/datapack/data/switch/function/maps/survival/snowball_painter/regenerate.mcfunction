...
execute if score #rg_snowball_painter switch.data matches 69.. run tellraw @a ["",{"nbt":"ParalyaWarning","storage":"switch:main","interpret":true},{"text":" Map '","color":"yellow"},{"text":"snowball_painter","color":"gold"},{"text":"' regenerated in ","color":"yellow"},{"text":"3","color":"gold"},{"text":"s","color":"yellow"}]

execute if score #rg_snowball_painter switch.data matches 69.. run data modify storage switch:main MessageToLog set value '{"text":"Map `snowball_painter` just regenerated!"}'

...
execute if score #rg_remelta_cluedo switch.data matches 1 run scoreboard players set #rg_remelta_cluedo_y switch.data 0
...
execute if score #rg_remelta_cluedo switch.data matches 130.. in switch:game run kill @e[type=item,x=118044,y=0,z=118033,distance=..1000]
...
execute if score #rg_remelta_cluedo switch.data matches 130.. run tellraw @a ["",{"nbt":"ParalyaWarning","storage":"switch:main","interpret":true},{"text":" Map '","color":"yellow"},{"text":"remelta_cluedo","color":"gold"},{"text":"' regenerated in ","color":"yellow"},{"text":"6","color":"gold"},{"text":"s","color":"yellow"}]

execute if score #rg_remelta_cluedo switch.data matches 130.. run data modify storage switch:main MessageToLog set value '{"text":"Map `remelta_cluedo` just regenerated!"}'

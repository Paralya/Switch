
#> switch:translations/modes_block_party_start
#
# @within	switch:modes/block_party/start
#

# French
tellraw @a[scores={switch.lang=0},tag=!detached] ["\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Lancement de la partie de Block Party dans 10 secondes, dirigez-vous vers la couleur indiquée dans le temps imparti !"}]

# English
tellraw @a[scores={switch.lang=1},tag=!detached] ["\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Block Party game starts in 10 seconds, head to the indicated color in the allotted time!"}]


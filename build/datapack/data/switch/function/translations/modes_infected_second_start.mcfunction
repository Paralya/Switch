...
execute if score #infected_seconds switch.data matches 0 run tellraw @a[scores={switch.lang=0},tag=!detached] [{"text":"La partie commence ! Cliquez [ici] si vous ne souhaitez pas être le premier infecté !","color":"yellow","clickEvent":{"action":"run_command","value":"/trigger switch.temp.dont_want_to_be_infected set 1"},"hoverEvent":{"action":"show_text","value":{"text":"Vous ne serez pas pris au hasard de la première infection !","color":"gray"}}}]
...
execute if score #infected_seconds switch.data matches 0 run tellraw @a[scores={switch.lang=1},tag=!detached] [{"text":"The game is starting! Click [here] if you don't want to be the first to be infected!","color":"yellow","clickEvent":{"action":"run_command","value":"/trigger switch.temp.dont_want_to_be_infected set 1"},"hoverEvent":{"action":"show_text","value":{"text":"You won't be caught in the randomness of the first infected!","color":"gray"}}}]

...
#> switch:modes/race/checkpoints/tick
#
# @executed	at @s
#
...
scoreboard players operation #x switch.data = @s switch.temp.x
scoreboard players operation #y switch.data = @s switch.temp.y
scoreboard players operation #z switch.data = @s switch.temp.z
...
execute as @a[tag=!detached,gamemode=!spectator,distance=..20,predicate=switch:has_same_checkpoint] run function switch:modes/race/checkpoints/check_player_pos

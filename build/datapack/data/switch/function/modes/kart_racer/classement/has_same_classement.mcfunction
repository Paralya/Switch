...
execute at @e[type=marker,tag=switch.checkpoint,predicate=switch:has_same_checkpoint,sort=nearest,limit=1] as @a[tag=!detached,tag=switch.process,sort=nearest] run function switch:modes/kart_racer/classement/increase

...
execute unless score #test_mode switch.data matches 1 as @a[tag=!detached,team=switch.temp.gays,gamemode=adventure] at @s if entity @s[y=105,dy=-1000] if data entity @s Inventory[{id:"minecraft:bow"}] run advancement grant @s only switch:visible/76

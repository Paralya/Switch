...
#> switch:modes/laser_game/second
#
# @within	switch:modes/laser_game/calls/second
#
...
execute if score #laser_game_seconds switch.data matches 10 if data storage switch:main {map:"laser_game"} in switch:game run summon item_frame 516 104 523 {Tags:["switch.laser_game.base"],Facing:2b,Invisible:true,Invulnerable:true,Fixed:true,Silent:true,Glowing:true}


# Base reload
scoreboard players remove #base_reload switch.data 1
function switch:translations/modes_laser_game_second
execute if score #base_reload switch.data matches 0 in switch:game run setblock 516 104 523 iron_trapdoor[powered=true,open=true]
...
execute if score #base_reload switch.data matches 1.. in switch:game run setblock 516 104 523 dark_oak_trapdoor[powered=true,open=true]

...
execute if score ${y} iris = $y0 iris if score ${x} iris >= $x0 iris if score ${x} iris <= $x1 iris if score ${z} iris >= $z0 iris if score ${z} iris <= $z1 iris run function iris:find_closest_surface/ray_already_in_surface

# If the current position is before the plane, check if it hits the surface and get the position of that intersection
execute if score ${y} iris < $y0 iris run function iris:find_closest_surface/bottom/get_distance

# Loop this function
data remove storage iris:data List[-1]
scoreboard players remove $surfaces iris 1
scoreboard players remove $cuboid_id iris 1
execute if score $surfaces iris matches 1.. run function iris:find_closest_surface/bottom/loop
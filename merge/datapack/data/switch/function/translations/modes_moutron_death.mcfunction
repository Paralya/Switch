...
execute if entity @s[gamemode=!spectator] if entity @p[tag=switch.moutron_killer] run tellraw @a[scores={switch.lang=0},tag=!detached] [{"selector":"@s","color":"red"},{"text":" s'est mangé en pleine face la trainée de "},{"selector":"@p[tag=switch.moutron_killer]","underlined":true},{"text":", survivant ainsi "},{"score":{"name":"#moutron_seconds","objective":"switch.data"}},{"text":" secondes !"}]
...
execute if entity @s[gamemode=!spectator] if entity @p[tag=switch.moutron_killer] run tellraw @a[scores={switch.lang=1},tag=!detached] [{"selector":"@s","color":"red"},{"text":" got hit by "},{"selector":"@p[tag=switch.moutron_killer]","underlined":true},{"text":"'s trail, survived "},{"score":{"name":"#moutron_seconds","objective":"switch.data"}},{"text":" seconds!"}]

...
def main(config: dict) -> None:
	...
	# On file
	write_function(config, FUNCTION_PATH, f"""
# Particles
""")
	for i in range(N_POINTS):
		...
		write_function(config, FUNCTION_PATH, f"particle dust{{color:[0.0,1.0,0.0],scale:1.0}} ~{round(x, ROUND_PRECISION)} ~ ~{round(z, ROUND_PRECISION)} 0 0 0 0 5 force\n")
	write_function(config, FUNCTION_PATH, f"""particle dust{{color:[0.0,0.75,0.0],scale:2.0}} ~ ~ ~ 2 0 2 0 10 force

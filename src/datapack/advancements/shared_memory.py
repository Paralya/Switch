...
# Function paths, relative to the project namespace (f"{ns}:{LOAD_FILE}")
LOAD_FILE: str = "advancements/_load"
...
{"id": "minecraft:stone", "components": {"minecraft:item_model": "{ns}:ultimate_bow"}}	48	22	OfChara	hard	Homophobe	Mettre au moins 42 flèches en Guy Shooter	Put at least 42 arrows in Guy Shooter
...
{"id": "minecraft:stone", "components": {"minecraft:item_model": "{ns}:ultimate_fragment"}}	categories/hard	30	Lawtank	hard	Légende	Gagnez 5 mini-jeux d'affilée	Win 5 mini-games in a row
...
{"id": "minecraft:stone", "components": {"minecraft:item_model": "{ns}:stardust_ingot"}}	53	54	Stoupy	hard	Secret Slayer	Terminer les secrets de toutes les maps dans le mode de jeu Infected	Complete all map secrets in Infected game mode
...
{"id": "minecraft:stone", "components": {"minecraft:item_model": "{ns}:awakened_stardust_bow"}}	40	58	Stoupy	medium	Harceleur	Eliminer 3 fois la même personne en Pitchout	Eliminate the same person 3 times in Pitchout
...
{"id": "minecraft:stone", "components": {"minecraft:item_model": "{ns}:ultimate_sniper"}}	22	76	OfChara	hard	Injustice	Finir le parcours en Guy Shooter mais mourir de la main d'un Shooter	Finish the parkour in Guy Shooter but die from the hands of a Shooter
{"id": "minecraft:stone", "components": {"minecraft:item_model": "{ns}:stardust_sniper"}}	72	77	Stoupy	easy	Pour l'équipe !	Tirer sur la base de bonus dans une partie de Laser Game	Shoot the bonus base in Laser Game
...
def generate_adv_dictionnary() -> None:
	...
	ALL_ADVANCEMENTS.extend([convert_line_to_dict(line) for line in GOOGLE_SHEET.replace("{ns}", ns).strip().split("\n")])

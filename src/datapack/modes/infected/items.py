...
def setup_infected_items() -> None:
	...
	for i, sword in enumerate(SWORDS):
		...
		Item(id=f"infected_sword_{i}", base_item="minecraft:iron_sword", components={
			"rarity": "epic", "item_model": sword_model, "item_name": f"{material} Sword",
	...
	for i, armor in enumerate(CHESTPLATES):
		...
		armor_attribute: list[dict[str, Any]] = [{
			"type": "armor", "slot": "chest", "id": f"{ns}.armor",
			"amount": 4.00 + (0.05 * i), "operation": "add_value"
		}]
		Item(
			...
			components={
				"max_stack_size": 1,
				"rarity": "epic",
				"equippable": {"slot": "chest", "asset_id": armor.replace("golden", "gold")},
	...
	for slot in ["head", "chest", "legs", "feet"]:
		Item(id=f"zombie_{slot}", components={
			"max_stack_size": 1,

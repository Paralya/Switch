...
def write_translations():
	...
	path: str = f"{ns}:modes/pitchout/translations"

	# /death
	write_function(f"{path}/death", f"""
# French
execute if entity @s[scores={{{ns}.alive=1}}] run tellraw @a[scores={{{ns}.lang=0}},tag=!detached] [{{"selector":"@s","color":"dark_red"}},{{"text":" est mort définitivement, il a survécu "}},{{"score":{{"name":"#pitchout_seconds","objective":"{ns}.data"}}}},{{"text":" secondes !"}}]
execute if entity @s[scores={{{ns}.alive=2}}] run tellraw @a[scores={{{ns}.lang=0}},tag=!detached] [{{"selector":"@s","color":"red"}},{{"text":" est tombé, il ne lui reste plus qu'une vie !"}}]
execute if entity @s[scores={{{ns}.alive=3}}] run tellraw @a[scores={{{ns}.lang=0}},tag=!detached] [{{"selector":"@s","color":"red"}},{{"text":" est tombé, il lui reste seulement 2 vies !"}}]

# English
execute if entity @s[scores={{{ns}.alive=1}}] run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] [{{"selector":"@s","color":"dark_red"}},{{"text":" died permanently, survived "}},{{"score":{{"name":"#pitchout_seconds","objective":"{ns}.data"}}}},{{"text":" seconds!"}}]
execute if entity @s[scores={{{ns}.alive=2}}] run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] [{{"selector":"@s","color":"red"}},{{"text":" has fallen, only one life left!"}}]
execute if entity @s[scores={{{ns}.alive=3}}] run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] [{{"selector":"@s","color":"red"}},{{"text":" has fallen, only 2 lives left!"}}]
""")

	# /in_water_with_cooldown
	write_function(f"{path}/in_water_with_cooldown", f"""
# French
tellraw @s[scores={{{ns}.lang=0}}] [{{"text":"Vous êtes tombé, mais vous aviez encore votre bouclier donc vous ne perdez pas de vie","color":"yellow"}}]

# English
tellraw @s[scores={{{ns}.lang=1}}] [{{"text":"You fell, but you still had your shield so you don't lose any life","color":"yellow"}}]
""")

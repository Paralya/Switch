...
def write_translations():
	""" Write the layers_4_teams translation functions at switch:modes/layers_4_teams/translations/* """
	ns: str = Mem.ctx.project_id
	path: str = f"{ns}:modes/layers_4_teams/translations"
...
execute if score #game_state {ns}.data matches 3 run tellraw @a[scores={{{ns}.lang=0}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Fin de partie, victoire des "}},{{"text":"verts","color":"green"}},{{"text":" ! GG à "}},{{"selector":"@a[tag=!detached,gamemode=!spectator,team={ns}.temp.green]"}}]
execute if score #game_state {ns}.data matches 4 run tellraw @a[scores={{{ns}.lang=0}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Fin de partie, victoire des "}},{{"text":"jaunes","color":"yellow"}},{{"text":" ! GG à "}},{{"selector":"@a[tag=!detached,gamemode=!spectator,team={ns}.temp.yellow]"}}]
execute if score #game_state {ns}.data matches 5 run tellraw @a[scores={{{ns}.lang=0}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Fin de partie, aucun des camps n'est sorti vainqueur car il n'y a plus aucun joueur en vie !"}}]
...
execute if score #game_state {ns}.data matches 1 run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Game over, winner is team "}},{{"text":"red","color":"red"}},{{"text":"! GG to "}},{{"selector":"@a[tag=!detached,gamemode=!spectator,team={ns}.temp.red]"}}]
execute if score #game_state {ns}.data matches 2 run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Game over, winner is team "}},{{"text":"blue","color":"blue"}},{{"text":"! GG to "}},{{"selector":"@a[tag=!detached,gamemode=!spectator,team={ns}.temp.blue]"}}]
execute if score #game_state {ns}.data matches 3 run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Game over, winner is team "}},{{"text":"green","color":"green"}},{{"text":"! GG to "}},{{"selector":"@a[tag=!detached,gamemode=!spectator,team={ns}.temp.green]"}}]
execute if score #game_state {ns}.data matches 4 run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Game over, winner is team "}},{{"text":"yellow","color":"yellow"}},{{"text":"! GG to "}},{{"selector":"@a[tag=!detached,gamemode=!spectator,team={ns}.temp.yellow]"}}]
execute if score #game_state {ns}.data matches 5 run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] ["\\n",{{"nbt":"Paralya","storage":"{ns}:main","interpret":true}},{{"text":" Game over, neither side has won because there are no players left alive!"}}]
""")

	# /second
	write_function(f"{path}/second", f"""
# French
execute if score #layers_4_teams_seconds {ns}.data matches 30 run tellraw @a[scores={{{ns}.lang=0}},tag=!detached] {{"text":"Les murs d'obsidienne ne sont plus indestructibles !","color":"red"}}

# English
function {ns}:translations/common/basic_actionbar
execute if score #layers_4_teams_seconds {ns}.data matches 30 run tellraw @a[scores={{{ns}.lang=1}},tag=!detached] {{"text":"The obsidian walls are no longer indestructible!","color":"red"}}
""")

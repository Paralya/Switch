...
def write_resources() -> None:
	...
	switch.advancements["race/inventory_changed"] = set_json_encoder(Advancement({
		...
		"rewards": {"function": f"{ns}:modes/race/advancements/inventory_changed"},
	}))
	switch.advancements["kart_racer/inventory_changed"] = set_json_encoder(Advancement({
		"criteria": {"requirement": {"trigger": "minecraft:inventory_changed"}},
		"rewards": {"function": f"{ns}:modes/kart_racer/advancements/inventory_changed"},
	}))

	# Predicates
	switch.predicates["has_same_classement"] = set_json_encoder(Predicate(same_score(f"{ns}.temp.classement", "#predicate")))
	switch.predicates["has_same_respawn_cp_id"] = set_json_encoder(Predicate(same_score(f"{ns}.temp.id", "#respawn_cp_id")))

...
def write_translations():
	...
	path: str = f"{Mem.ctx.project_id}:modes/sheepwars/translations"
...
execute if score #game_state switch.data matches 1 run tellraw @a[scores={switch.lang=1},tag=!detached] ["\\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Game over, victory of "},{"text":"red","color":"red"},{"text":" team! GG to "},{"selector":"@a[tag=!detached,gamemode=!spectator,team=switch.temp.red]"}]
execute if score #game_state switch.data matches 2 run tellraw @a[scores={switch.lang=1},tag=!detached] ["\\n",{"nbt":"Paralya","storage":"switch:main","interpret":true},{"text":" Game over, victory of "},{"text":"blue","color":"blue"},{"text":" team! GG to "},{"selector":"@a[tag=!detached,gamemode=!spectator,team=switch.temp.blue]"}]
...
""")
	...
	# /magic_wool_place
	write_function(f"{path}/magic_wool_place", """
# French
tellraw @a[scores={switch.lang=0},tag=!detached] ["",{"text":"[SheepWars] ","color":"yellow"},{"text":"Une laine magique est apparue !"}]

# English
tellraw @a[scores={switch.lang=1},tag=!detached] ["",{"text":"[SheepWars] ","color":"yellow"},{"text":"A magical wool has appeared!"}]
""")

	# /magic_wool_slot
	write_function(f"{path}/magic_wool_slot", """
# French
...
execute if score #random switch.data matches 0 run tellraw @a[scores={switch.lang=1},tag=!detached] {"text":"[20s of resistance for their team]\\n","color":"aqua"}
execute if score #random switch.data matches 1 run tellraw @a[scores={switch.lang=1},tag=!detached] {"text":"[20s regeneration for their team]\\n","color":"aqua"}
...
execute if score #random switch.data matches 4 run tellraw @a[scores={switch.lang=1},tag=!detached] {"text":"[One drop of sheep for their team]\\n","color":"aqua"}
execute if score #random switch.data matches 5 run tellraw @a[scores={switch.lang=1},tag=!detached] {"text":"[10s of flaming arrows for their team]\\n","color":"aqua"}
execute if score #random switch.data matches 6 run tellraw @a[scores={switch.lang=1},tag=!detached] {"text":"[10s of explosive arrows for their team]\\n","color":"aqua"}
execute if score #random switch.data matches 7 run tellraw @a[scores={switch.lang=1},tag=!detached] {"text":"[20s of slow falling for their team]\\n","color":"aqua"}
